...
export default function DashboardPage({ onLogout }: Props) {
  ...
  const handleCreate = async () => {
    ...
    setError('');
    try {
      await api.createPersonalDevice('Personal Device');
      await fetchDevices();
    } catch (err: unknown) {
  ...
  const handleDelete = async (email: string) => {
    if (!confirm(`Delete device ${email}?`)) return;
    setError('');
    try {
  ...
  return (
    <div className="min-h-screen p-4">
      <div className="mx-auto max-w-2xl">
        ...
        >
          {creating ? 'Creating...' : '+ Add Personal Device'}
        </button>
              ...
              >
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    ...
                    {d.created_at && ` | Created: ${d.created_at.slice(0, 10)}`}
                  </div>
                  <div className="mt-1">
                    <span
